...
structured_statline22 <- function(data, opp_pred, pos) {
  ...
  if (pos == "QB") {
    ...
    result$projected_targets <- 0; result$projected_receptions <- 0; result$projected_rec_yards <- 0; result$projected_rec_tds <- 0
  } else {
    ...
    result$projected_pass_attempts <- 0; result$projected_pass_yards <- 0; result$projected_pass_tds <- 0; result$projected_interceptions <- 0
    if (pos %in% c("RB", "WR")) {
      ...
      result$projected_carries <- 0; result$projected_rush_yards <- 0; result$projected_rush_tds <- 0
    }
  }
