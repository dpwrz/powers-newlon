...
if (all_ok) {
  ...
  cat("Next: source(\"runners/MOBILE_RUN_FAST.R\")\n")
} else {
  cat("FAIL: One or more prerequisites are missing.\n")
  cat("If only PBP context is missing, run source(\"pipeline/BUILD_PBP_CONTEXT.R\").\n")
  cat("For a clean rebuild, run source(\"runners/MOBILE_RUN.R\").\n")
