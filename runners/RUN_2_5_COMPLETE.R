...
# Full guarded Model 2.5 build: validate first, then project only promoted positions.
source("config.R")
ensure_packages(c("dplyr", "readr", "xgboost"))
cat("\n=== FANTASY MODEL 2.5 - GUARDED COMPLETE BUILD ===\n")
source("pipeline/22_train_validate_accuracy_tournament_2_5.R")
source("pipeline/23_project_weekly_2026_2_5.R")
cat("\n=== MODEL 2.5 COMPLETE ===\n")
cat("Current weekly grid: output/weekly_", CURRENT_SEASON, "_projections.csv\n", sep = "")
cat("ROS: output/rest_of_season_", CURRENT_SEASON, ".csv\n", sep = "")
cat("Champion manifest: output/weekly_2_5_champion_manifest.csv\n")
