# ============================================================
# FANTASY MODEL 2.2 - OPTIONAL WEEKLY ML ENGINE BENCHMARK
# ============================================================
# This script never changes production models. It compares optional engines on
# the same outer season walk-forward used by 2.2's full-direct challenger.
source("config.R")
...
cat("[2.2 ENGINE BENCHMARK] Available engines: ", paste(engines, collapse = ", "), "\n", sep = "")
if (length(engines) == 1) cat("[2.2 ENGINE BENCHMARK] Optional ranger/xgboost are not installed; reporting rpart reference only.\n")
...
for (pos in POSITIONS) {
  ...
  for (yr in validation_years) {
    ...
    for (engine in engines) {
      cat("[2.2 ENGINE BENCHMARK] ", yr, " ", pos, " ", engine, "\n", sep = "")
...
readr::write_csv(preds, "output/weekly_2_2_engine_benchmark_predictions.csv")
readr::write_csv(metrics, "output/weekly_2_2_engine_benchmark.csv")
cat("\n[2.2 ENGINE BENCHMARK] Complete: output/weekly_2_2_engine_benchmark.csv\n")
print(metrics)
