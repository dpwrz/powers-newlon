...
if (nzchar(rscript)) {
  status <- system2(rscript, args = c("RUN_STAGE_20.R", "02b_build_opportunity_data.R", log_path), stdout = "", stderr = "")
  ...
  source("02b_build_opportunity_data.R")
}

cat("\nPASS: Fantasy Model 2.0 opportunity/shrinkage feature store completed.\n")
cat("Next run: source(\"MOBILE_RUN_FAST.R\")\n\n")
