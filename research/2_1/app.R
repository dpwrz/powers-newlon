# ============================================================
# FANTASY MODEL 2.1 - SEASON + WEEKLY MOBILE SHINY APPLICATION
# ============================================================
source("config.R")
...
weekly_quality_report <- if (file.exists(weekly_quality_path)) readLines(weekly_quality_path, warn = FALSE) else "Run source(\"MOBILE_RUN_WEEKLY.R\") to generate weekly validation and projections."
...
weekly_mobile_card <- function(d) {
  delta <- suppressWarnings(as.numeric(d$matchup_delta_vs_prior))
  matchup <- if (!is.finite(delta)) "Neutral" else if (delta >= 1.5) "Favorable" else if (delta <= -1.5) "Difficult" else "Neutral"
  div(
    class = "fm-player-row",
    div(class = "fm-rank-badge", paste0("#", d$weekly_position_rank)),
    div(class = "fm-player-main",
        div(class = "fm-player-name", d$player_display_name),
        div(class = "fm-player-meta", paste0(d$position, " · ", d$team, " vs ", d$opponent, " · ", matchup))),
    div(class = "fm-player-score",
        strong(num1(d$projected_weekly_fppg)),
        span(" pts"),
        tags$small(paste0("Δ ", sprintf("%+.1f", ifelse(is.finite(delta), delta, 0)))))
  )
}
...
ui <- navbarPage(
  ...
  tabPanel(
    "Home",
    div(class = "fm-hero",
        h2("Fantasy Model 2.1"),
        p(class = "fm-muted", "Validated season engine + strict pre-kickoff weekly matchup projections + rest-of-season forecasting.")),
    fluidRow(
    ...
    fluidRow(
      ...
      column(5, div(class = "fm-card",
                    h4("What changed in 2.1"),
                    tags$ul(
                      tags$li("Adds strict pre-kickoff week-by-week projections while keeping the validated 2.0 season engine as the preseason prior."),
                      tags$li("Player role updates from recent fantasy production, targets, carries, pass attempts and offensive snap share."),
                      tags$li("Opponent strength is adjusted for what opposing players were expected to score, not raw fantasy points allowed alone."),
                      tags$li("Compact PBP defense profiles model pass/rush EPA, success, pressure, explosive plays, depth/location and red-zone tendencies."),
                      tags$li("Schedule context adds home/away, rest, betting total/spread and implied team scoring environment when available."),
                      tags$li("Every historical holdout is rebuilt using only information available before that week's kickoff." )
                    ),
                    p(class = "fm-muted", "Weekly projections and rest-of-season totals update as new role and matchup information arrives; the season model remains the long-term prior.")))
    )
  ),
  ...
  tabPanel(
    "Weekly",
    div(class = "fm-hero", h2("Weekly Projections"), p(class = "fm-muted", "Pregame projections using current role, opponent-adjusted defense, compact PBP matchup context, snap share, rest, game environment and availability data when available.")),
    fluidRow(
      column(3, selectInput("weekly_week", "Week", choices = as.character(1:18), selected = as.character(current_week_value))),
      column(3, selectInput("weekly_position", "Position", c("ALL", "QB", "RB", "WR", "TE"))),
      column(6, textInput("weekly_search", "Player search", placeholder = "Type a player name..."))
    ),
    uiOutput("weekly_status"),
    fluidRow(
      column(4, uiOutput("weekly_top_player")),
      column(4, uiOutput("weekly_best_matchup")),
      column(4, uiOutput("weekly_week_summary"))
    ),
    div(class = "fm-card",
        div(class = "fm-section-head", h4("Week rankings"), span(class = "fm-pill", "Default half-PPR model scoring")),
        div(class = "fm-desktop-only fm-table-wrap", tableOutput("weekly_table")),
        div(class = "fm-mobile-only", uiOutput("weekly_cards"))),
    div(class = "fm-card", h4("Rest-of-season leaders"), div(class = "fm-table-wrap", tableOutput("ros_table")))
  ),
  ...
  tabPanel(
    ...
    div(class = "fm-card", h4("Projection range"), uiOutput("player_range_bar")),
  ...
  tabPanel(
    "Quality",
    div(class = "fm-hero", h2("Model Quality"), p(class = "fm-muted", "Season Model 2.0 plus the new 2.1 weekly engine are evaluated separately on honest historical holdouts.")),
    fluidRow(column(12, div(class = "fm-card", h4("2.0 quality report"), verbatimTextOutput("quality_report")))),
    fluidRow(
      column(6, div(class = "fm-card", h4("Walk-forward regression"), div(class = "fm-table-wrap", tableOutput("validation_table")))),
      column(6, div(class = "fm-card", h4("Decision models"), div(class = "fm-table-wrap", tableOutput("probability_table"))))
    ),
    fluidRow(column(12, div(class = "fm-card", h4("2.0 vs 1.2"), div(class = "fm-table-wrap", tableOutput("version_comparison_table"))))),
    fluidRow(column(12, div(class = "fm-card", h4("2.1 weekly quality report"), verbatimTextOutput("weekly_quality_report")))),
    fluidRow(column(12, div(class = "fm-card", h4("2.1 weekly walk-forward metrics"), div(class = "fm-table-wrap", tableOutput("weekly_validation_table")))))
  )
)
...
server <- function(input, output, session) {
  ...
  weekly_filtered <- reactive({
    ...
    d <- weekly_base |> dplyr::filter(week == as.integer(input$weekly_week %||% current_week_value), is_actual == 0)
    ...
    d |> dplyr::arrange(dplyr::desc(projected_weekly_fppg))
  })

  output$weekly_status <- renderUI({
    if (nrow(weekly_base) == 0) return(div(class = "fm-card", strong("Weekly engine not generated yet."), p(class = "fm-muted", "Run source(\"MOBILE_RUN_WEEKLY.R\") once, then relaunch the app.")))
    NULL
  })
  output$weekly_top_player <- renderUI({
    d <- weekly_filtered(); if (nrow(d) == 0) return(kpi("Top projection", "—", "No projection rows"))
    x <- d |> dplyr::slice(1); kpi(paste0("Week ", x$week, " #1"), x$player_display_name, paste0(x$position, " · vs ", x$opponent, " · ", num1(x$projected_weekly_fppg), " pts"), TRUE)
  })
  output$weekly_best_matchup <- renderUI({
    d <- weekly_filtered(); if (nrow(d) == 0) return(kpi("Best matchup boost", "—"))
    x <- d |> dplyr::arrange(dplyr::desc(matchup_delta_vs_prior)) |> dplyr::slice(1)
    kpi("Best matchup boost", x$player_display_name, paste0(sprintf("%+.1f", x$matchup_delta_vs_prior), " vs role/prior baseline"))
  })
  output$weekly_week_summary <- renderUI({
    ...
    kpi("Projection week", paste0("Week ", wk), paste0(nrow(d), " ranked player rows"))
  })
  output$weekly_table <- renderTable({
    ...
    d |> dplyr::slice_head(n = 150) |> dplyr::transmute(
      Rank = weekly_position_rank, Player = player_display_name, Pos = position, Team = team, Opp = opponent,
      Projection = round(projected_weekly_fppg, 1), Floor = round(weekly_floor, 1), Ceiling = round(weekly_ceiling, 1),
      `Matchup Δ` = sprintf("%+.1f", matchup_delta_vs_prior), Injury = ifelse(injury_risk > 0, injury_status, "—")
    )
  }, striped = TRUE, bordered = FALSE, hover = TRUE, spacing = "s", rownames = FALSE)
  ...
  output$ros_table <- renderTable({
    if (nrow(weekly_ros_base) == 0) return(data.frame(Message = "Run the weekly engine to generate rest-of-season projections."))
    weekly_ros_base |> dplyr::slice_head(n = 30) |> dplyr::transmute(Player = player_display_name, Pos = position, Team = team, `Actual to date` = round(actual_points_to_date, 1), `Projected remaining` = round(projected_remaining_points, 1), `Full season` = round(projected_full_season_points, 1), `ROS Pos Rank` = ros_position_rank)
  }, striped = TRUE, bordered = FALSE, spacing = "s", rownames = FALSE)
  ...
  output$weekly_validation_table <- renderTable({
    if (nrow(weekly_validation_base) == 0) return(data.frame(Message = "Run the 2.1 weekly engine to generate weekly validation."))
    weekly_validation_base |> dplyr::transmute(Pos = position, N = n, `Weekly MAE` = round(final_MAE, 2), `Prior MAE` = round(baseline_MAE, 2), `MAE Gain` = paste0(round(MAE_improvement_vs_prior_pct, 1), "%"), RMSE = round(final_RMSE, 2), Corr = round(correlation, 3), `Rank Corr` = round(rank_correlation, 3), Weight = round(weekly_model_weight, 2))
  }, striped = TRUE, bordered = FALSE, spacing = "s", rownames = FALSE)
